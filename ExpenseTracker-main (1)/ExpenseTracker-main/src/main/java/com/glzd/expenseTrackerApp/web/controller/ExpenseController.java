...
import com.glzd.expenseTrackerApp.business.services.AdminServiceImpl;
...
public class ExpenseController {
    ...
    public String registerCustomer(@Valid @ModelAttribute("adminDto") AdminDto adminDto,
                                   BindingResult result,
                                   Model model) {
      System.out.println("register "+ adminDto.getEmailAddress());
        try {
            if (result.hasErrors()) {
                model.addAttribute("adminDto",new AdminDto());
                model.addAttribute("error", "please check your inputs fields.\npassword should contain 8-10 characters");
                System.out.println("first step");
                return "main";
            }
            String username = adminDto.getEmailAddress();
            Admin customer = adminServiceImpl.findByUsername(username);
            if (customer != null) {
                model.addAttribute("adminDto", adminDto);
                System.out.println("second step");
                model.addAttribute("error", "we already have this email please change!");
                return "main";
            }
    ...
    @GetMapping("/do-login")
    public String loginMedthod(@Valid @ModelAttribute("adminDto") AdminDto adminDto,
                                   BindingResult result,
                                   Model model) {
        try {
            if (result.hasErrors()) {
            ...
            System.out.println("username+++++++++++++++++++++++++++++++++++" + username);
                Admin user = adminServiceImpl.findByUsername(username);
            System.out.println(" after username+++++++++++++++++++++++++++++++++++" + username);
             if (user != null) {
                ...
                if(passwordEncoder.matches(adminDto.getPassword(),user.getPassword())){
                    ...
                    model.addAttribute("error", "wrong password please try again later");
                    return "main";
                }
            }
            return "main";
    ...
    @GetMapping("/")
    public String openFirst(Model model){
        model.addAttribute("adminDto", new AdminDto());
        return "main";
    }
